...
class Events(commands.Cog):   
        ...
        async def on_message(self, message):
                if self.client.user.mentioned_in(message):
                        embed = discord.Embed(title = f"Prefix {config.prefix}", description=f"Hey! my names {self.client.user} you can check my commands list with {config.prefix}Help at anytime!",color=0x9B59B6,)
